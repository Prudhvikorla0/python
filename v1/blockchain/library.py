...
@shared_task(name='post_blockchain_request')
def post_blockchain_request(request_id):
    """
    Function to post the request to the blockchain middleware server.
    BlockchainRequest is fetched evey time to update values because,
    in most situations, the callback might be received before this function is
    completed, some of the values might have changed after the object is fetched
    from the db at the start of the function.
    Updating using existing object reverts these values. to avoid this, the update
    is done at the database level soon after the objects are fetched
    """
    ...
    try:
        ...
        BlockchainRequest.objects.filter(
            id=request_id).update(
            response=response_json)
        if not response.ok:
            BlockchainRequest.objects.filter(
                id=request_id).update(status=constants.BC_REQUEST_STATUS_FAILED)
            base_request.callback_token.invalidate()
            response.raise_for_status()
        BlockchainRequest.objects.filter(
            id=request_id).update(
            receipt=response_json['data']['receipt'], last_api_call=datetime.now())
