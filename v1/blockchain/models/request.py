...
class BlockchainRequest(models.Model):
    ...
    Attributes:
        body                : To temporarily store the body before sending the request
        header              : To temporarily store the header before sending the request
    ...
    callback_token = models.OneToOneField(
        CallBackToken, related_name='request', default=None,
    ...
    response = models.JSONField(default=dict, encoder=DjangoJSONEncoder)
    ...
    def save(self, *args, **kwargs):
        ...
        super(BlockchainRequest, self).save(*args, **kwargs)
    ...
    def check_balance_low(response):
        ...
        """
        if 'message' in response and 'INSUFFICIENT_PAYER_BALANCE' in response['message']:
            return True, True
        if 'data' in response and type(response['data']) is dict:
            if 'initiatorBal' in response['data']:
                if response['data']['initiatorBal'] <= constants.HBAR_MINIMUM_BALANCE:
                    return True, False
        return False, False

    def prepare_header(
            self, public=settings.TREASURY_ACCOUNT_ID,
            private=settings.TREASURY_ENCRYPED_PRIVATE):
        """
        The request to the blockchain node is signed using the private key issued
        from the blockchain node manager. The blockchain key that takes the action
        is also sent in the context.
        This function formats the header accordingly.
        """
        context = {
            'private': private,
            'public': public,
        ...
        auth = APIAuth(
            access_key_id=settings.BLOCKCHAIN_CLIENT_ID,
            context=context)
    ...
    def prepare_body(self):
        ...
        """
        body = {
            'callback_url': f"{settings.ROOT_URL}{reverse('update-hash')}"
                            f"?token={self.callback_token.key}&salt={self.callback_token.idencode}",
            'action': self.action,
    ...
    def send(self):
        ...
        self.save()
        transaction.on_commit(lambda: library.post_blockchain_request(request_id=self.id))
        return True

    def manage_callback(self, response):
        """
        Function to manage the callback.
        updates the status of the request, marks the callback token as used and
        calls the handle_response of subclass.
        """
        try:
    ...
    def handle_response(self, response):
        """
        This function should be implemented in all classes that inherits from
        this baseclass to handle the request response and update the blockchain
        hash to the appropriate attribute.
        """
    ...
    def is_delayed(self):
        if self.status == constants.BC_REQUEST_STATUS_PENDING:
            if self.updated_on < timezone.now() - timedelta(**constants.REQUEST_DELAY_TOLERENCE):
                return True
        return False
