...
country_file_path = os.path.join(
    settings.BASE_DIR, 'common', 'country_data', 'country_data_with_province_latlong.json')

with open(country_file_path) as country_file:
    COUNTRIES = json.loads(country_file.read())
    COUNTRY_LIST = list(COUNTRIES.keys())
    COUNTRY_WITH_PROVINCE = {i: list(v['sub_divisions'].keys()) for i, v in COUNTRIES.items()}
    DIAL_CODES = ['+' + i['dial_code'] for i in COUNTRIES.values()]

    DIAL_CODE_NAME_MAP = {'+' + val['dial_code']:'%s (+%s)' % (k, val['dial_code']) for k, val in COUNTRIES.items()}
