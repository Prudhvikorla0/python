...
class IdencodeField(serializers.CharField):
    ...
    def __init__(self, serializer=None, related_model=None, prefetch: list = None, *args, **kwargs):
    ...
    def to_representation(self, value):
        ...
        if not value:
            return None
        if self.prefetch and self.related_model:
            value = self.related_model.objects.prefetch_related(
                *self.prefetch).filter(id=value.id).first()
        if self.serializer:
